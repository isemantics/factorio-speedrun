...
if not speedrun.value_sensors then speedrun.value_sensors = {} end
...
function ValueSensor.new(name)
    ...
    function sensor:update_ui(owner)
        ...
        local sensor_settings = global.speedrun[player.name].sensor_settings[self.name]
    ...
    function sensor:on_click(event)
        if string.starts_with(event.element.name, "speedrun_sensor_" .. self.name .. "_checkbox_") then
            local len = string.len("speedrun_sensor_" .. self.name .. "_checkbox_")
    ...
    function sensor:make_on_click_checkbox_handler(setting_name)
        ...
        return function(event)
            ...
            local sensor_settings = global.speedrun[player.name].sensor_settings[sensor_name]
...
function ValueSensor.register(sensor)
    table.insert(speedrun.value_sensors, sensor)
end
